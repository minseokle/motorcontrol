...
void pack_reply(CANTxMessage * msg, uint8_t id, float p, float v, float t, float vb)
{
  ...
  int t_int =
    float_to_uint(t, -(I_MAX + SENSE_BUFFER) * KT * GR, (I_MAX + SENSE_BUFFER) * KT * GR, 12);
...
{  // ControllerStruct * controller){
  ...
  // printf("Received   ");
  // printf("%.3f  %.3f  %.3f  %.3f  %.3f   %.3f", controller->p_des, controller->v_des,
  // controller->kp, controller->kd, controller->t_ff, controller->i_q_ref); printf("\n\r");
}
