    ...
    case CALIBRATION_MODE:
      if (!comm_encoder_cal.done_ordering) {
        ...
        if (ENCODER_TYPE == ENCODER_ABS) {
          calibrate_abs_encoder(
            &basic_encoder, &abs_encoder, &controller, &comm_encoder_cal, controller.loop_count);
        } else if (ENCODER_TYPE == ENCODER_HALL) {
          /* Hall sensor calibration */
          calibrate_hall_encoder(
            &basic_encoder, &hall_sensor, &controller, &comm_encoder_cal, controller.loop_count);
        }
      } else {
        ...
        printf(
          "E_ZERO: %d  %f\r\n", E_ZERO,
          TWO_PI_F * fmodf((abs_encoder.ppairs * (float)(-E_ZERO)) / ((float)ENC_CPR), 1.0f));
        if (ENCODER_TYPE == ENCODER_ABS) {
...
void fsm_exit_state(FSMStruct * fsmstate)
{
  ...
  switch (fsmstate->state) {
    ...
    case ENCODER_MODE:
      ...
      break;
    case MOTOR_MODE:
      /* Don't stop commuting if there are high currents or FW happening */
  ...
  switch (fsmstate->state) {
    ...
    case SETUP_MODE:
      ...
      if (fsmstate->bytecount == 0) {
        ...
        // fsmstate->bytecount =
        // fsmstate->bytecount%(sizeof(fsmstate->cmd_buff)/sizeof(fsmstate->cmd_buff[0])); // reset
        // when buffer is full
      }
      fsmstate->bytecount++;
...
void enter_setup_state(void)
{
  ...
  // printf(" %-4s %-31s %-5s %-6s %.1f\n\r", "h", "Temp Cutoff (C) (0 = none)", "0", "150",
  // TEMP_MAX);
  printf(
