...
void ps_warmup(AbsEncoderStruct * encoder, int n)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    HAL_SPI_TransmitReceive(
      &ENC_SPI, (uint8_t *)encoder->spi_tx_buff, (uint8_t *)encoder->spi_rx_buff, 1, 100);
...
void ps_sample(AbsEncoderStruct * encoder, float dt)
{
  ...
  // memmove(&encoder->angle_multiturn[1], &encoder->angle_multiturn[0],
  // (N_POS_SAMPLES-1)*sizeof(float)); // this is much slower for some reason
  ...
  // encoder->elec_angle =
  // TWO_PI_F*fmodf((encoder->ppairs*(float)(encoder->count-E_ZERO))/((float)ENC_CPR), 1.0f);
