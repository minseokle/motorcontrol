...
uint8_t hall_sensor_get_state_from_gpio()
{
  ...
  hall_state |= HAL_GPIO_ReadPin(HALL_U_GPIO_Port, HALL_U_Pin) << 0; // U
  hall_state |= HAL_GPIO_ReadPin(HALL_V_GPIO_Port, HALL_V_Pin) << 1; // V
  hall_state |= HAL_GPIO_ReadPin(HALL_W_GPIO_Port, HALL_W_Pin) << 2; // W
  return hall_state;
}
...
void hall_sensor_update(HallSensorStruct *h, uint8_t new_hall_state, uint16_t current_time_ticks)
{
  ...
  {
    h->electrical_velocity_rad_s = ((float)h->direction * PI_OVER_3_F) / ((float)delta_time_ticks * TIMER_TICK_S);
  }
  else
  {
    h->electrical_velocity_rad_s = 0.0f;
